...
function fail(message) {
  throw new Error(`invalid external brand proof evidence: ${message}`);
}

const configured = process.env.VIRA_EXTERNAL_BRAND_PROOF_EVIDENCE;
if (!configured) {
  throw new Error("VIRA_EXTERNAL_BRAND_PROOF_EVIDENCE must point to external brand proof evidence JSON");
}
...
try {
  ...
  throw new Error(
    `external brand proof evidence could not be read from ${path}: ${error instanceof Error ? error.message : String(error)}`,
    { cause: error },
  );
}
...
if (git.error || git.status !== 0) throw new Error("git rev-parse HEAD failed while verifying external brand proof evidence");
const currentHead = git.stdout.trim();
if (currentHead !== evidence.viraHead) {
  throw new Error(`external brand proof evidence targets ${evidence.viraHead}, but current checkout is ${currentHead}`);
}

console.log(`External brand proof evidence verified for ${evidence.pack.id}@${evidence.pack.version} (${evidence.pack.digest})`);
