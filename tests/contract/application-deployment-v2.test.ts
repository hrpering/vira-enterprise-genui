...
function binding(environment: "dev" | "staging" | "production", trustStatus: "trusted" | "untrusted" = "trusted"): ViraApplicationEnvironmentBinding {
  return {
    version: "1",
...
describe("PROD-05 authenticated Application deployment", () => {
  it("publishes an authenticated exact Application distribution to dev", async () => {
    const created = plane();
    ...
    expect(result.value.binding.scope.environment).toBe("dev");
  });
  ...
  it("rejects tampered distribution integrity", async () => {
    ...
    const tampered = {
      ...artifact,
      distribution: { ...artifact.distribution, integrity: { algorithm: "sha256" as const, digest: "0".repeat(64) } },
    };
    const result = await created.value.publish({ artifact: tampered, binding: binding("dev") });
    expect(result).toMatchObject({ ok: false, issue: { code: "DISTRIBUTION_INTEGRITY_FAILED" } });
  });

  it("rejects wrong publisher, unauthenticated provenance, and invalid signature", async () => {
    const created = plane();
    expect(created.ok).toBe(true);
    if (!created.ok) return;
  ...
  it("enforces immutable Application id/version to one distribution digest", async () => {
    const created = plane();
    expect(created.ok).toBe(true);
    if (!created.ok) return;
    expect((await created.value.publish({ artifact: signed(), binding: binding("dev") })).ok).toBe(true);
    const conflict = await created.value.publish({ artifact: signed(application("1.2.0", "Changed Flight Assistant")), binding: binding("dev") });
    expect(conflict).toMatchObject({ ok: false, issue: { code: "ARTIFACT_CONFLICT" } });
  });

  it("allows only adjacent exact release promotion with target-environment binding", async () => {
    const created = plane();
    expect(created.ok).toBe(true);
    if (!created.ok) return;
    const artifact = signed();
    ...
    const digest = artifact.distribution.integrity.digest;
    expect(await created.value.promote({ release, distributionDigest: digest, from: "dev", to: "production", binding: binding("production") }))
      .toMatchObject({ ok: false, issue: { code: "INVALID_PROMOTION" } });
    expect((await created.value.promote({ release, distributionDigest: digest, from: "dev", to: "staging", binding: binding("staging") })).ok).toBe(true);
    expect((await created.value.promote({ release, distributionDigest: digest, from: "staging", to: "production", binding: binding("production") })).ok).toBe(true);
  });

  it("rolls back to the exact historical release identity and binding", async () => {
    const created = plane();
    expect(created.ok).toBe(true);
    if (!created.ok) return;
    const firstArtifact = signed(application("1.2.0"));
    const first = await created.value.publish({ artifact: firstArtifact, binding: binding("dev") });
    expect(first.ok).toBe(true);
    if (!first.ok) return;
    const secondArtifact = signed(application("1.3.0"));
    expect((await created.value.publish({ artifact: secondArtifact, binding: binding("dev") })).ok).toBe(true);
  ...
  it("rejects cross-scope and untrusted environment bindings", async () => {
    ...
    const bad = {
      ...crossScope,
      secretRef: { ...crossScope.secretRef, projectId: "other-project" },
    };
  ...
  it("deprecation blocks promotion, rollback target and cached artifact acceptance", async () => {
    ...
    const input = { release: published.value.release, distributionDigest: published.value.distributionDigest };
    expect((await created.value.deprecate(input)).ok).toBe(true);
    expect(await created.value.promote({ ...input, from: "dev", to: "staging", binding: binding("staging") }))
      .toMatchObject({ ok: false, issue: { code: "ARTIFACT_DEPRECATED" } });
    expect(await created.value.rollback({ environment: "dev", deploymentId: published.value.deploymentId }))
      .toMatchObject({ ok: false, issue: { code: "ARTIFACT_DEPRECATED" } });
    expect(await created.value.verifyCachedApplication(artifact))
      .toMatchObject({ ok: false, issue: { code: "ARTIFACT_DEPRECATED" } });
  });
});
