...
describe("Experience Registry ambient prototype hardening", () => {
  ...
  it("looks up manifests without ambient descriptor, integer, or freeze intrinsics", () => {
    const snapshot = registry();
    const previousDescriptor = Object.getOwnPropertyDescriptor(Object, "getOwnPropertyDescriptor");
    const previousSafeInteger = Object.getOwnPropertyDescriptor(Number, "isSafeInteger");
    const previousFreeze = Object.getOwnPropertyDescriptor(Object, "freeze");
    let calls = 0;
    let hit: ReturnType<typeof lookupExperienceRegistryManifest>;
    let miss: ReturnType<typeof lookupExperienceRegistryManifest>;
    ...
    expect(hit).toMatchObject({ ok: true, value: { manifest: { id: "vira/minimal", version: "1.0.0" } } });
    expect(miss).toEqual({ ok: true, value: { manifest: null } });
    if (hit.ok) expect(Object.isFrozen(hit.value)).toBe(true);
    if (miss.ok) expect(Object.isFrozen(miss.value)).toBe(true);
  });
});
