...
describe("PROD-05 exact Application resolution", () => {
  it("resolves only the exact active release into an immutable canonical artifact digest", async () => {
    ...
    expect(createHash("sha256").update(resolved.value.canonicalArtifact).digest("hex"))
      .toBe(resolved.value.resolutionDigest);
  });
  ...
  it("rejects resolution from another enterprise scope", async () => {
    const { plane, publication } = await deployed();
    const resolver = createViraApplicationResolver({ source: plane, digest: () => "a".repeat(64) });
    expect(resolver.ok).toBe(true);
    if (!resolver.ok) return;
    const otherScope = { ...scope(), projectId: "other-project" };
    expect(await resolver.value.resolve({ release: publication.value.release, scope: otherScope }))
      .toMatchObject({ ok: false, issue: { code: "INVALID_SCOPE" } });
  });

  it("rejects a deprecated release even when the old deployment remains mounted", async () => {
    const { plane, publication } = await deployed();
    expect((await plane.deprecate({ release: publication.value.release, distributionDigest: publication.value.distributionDigest })).ok).toBe(true);
    const resolver = createViraApplicationResolver({ source: plane, digest: () => "a".repeat(64) });
    expect(resolver.ok).toBe(true);
    if (!resolver.ok) return;
    expect(await resolver.value.resolve({ release: publication.value.release, scope: scope() }))
      .toMatchObject({ ok: false, issue: { code: "APPLICATION_DEPRECATED" } });
  });

  it("fails closed on internally inconsistent cached/source deployment data", async () => {
    const { plane, publication } = await deployed();
    const candidate = plane.lookupActive({ release: publication.value.release, environment: "dev" });
    ...
    const conflicted = {
      ...
      deployment: { ...candidate.value.deployment, distributionDigest: "f".repeat(64) },
    };
    const resolver = createViraApplicationResolver({
      source: { lookupActive: () => ({ ok: true as const, value: conflicted }) },
      digest: () => "a".repeat(64),
    });
    expect(resolver.ok).toBe(true);
    if (!resolver.ok) return;
  ...
  it("fails closed when the deployment source throws", async () => {
    const resolver = createViraApplicationResolver({
      source: { lookupActive: () => { throw new Error("boom"); } },
      digest: () => "a".repeat(64),
    });
    expect(resolver.ok).toBe(true);
    if (!resolver.ok) return;
  ...
  it("follows rollback without mutating historical release identity", async () => {
    ...
    expect((await plane.rollback({ environment: "dev", deploymentId: publication.value.deploymentId })).ok).toBe(true);
    const resolver = createViraApplicationResolver({ source: plane, digest: (value) => createHash("sha256").update(value).digest("hex") });
    expect(resolver.ok).toBe(true);
    if (!resolver.ok) return;
    const resolved = await resolver.value.resolve({ release: publication.value.release, scope: scope() });
