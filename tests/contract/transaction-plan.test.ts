...
type PlanFixture = ReturnType<typeof plan>;
type OperationFixture = ReturnType<typeof operation>;

function firstOperation(candidate: PlanFixture): OperationFixture {
  const first = candidate.operations[0];
  if (first === undefined) throw new Error("transaction plan test fixture requires one operation");
  return first;
}

function firstPricingRef(candidate: PlanFixture): { id: string; versionRef: string } {
  const first = candidate.commercial.pricingRefs[0];
  if (first === undefined) throw new Error("transaction plan test fixture requires one pricing reference");
  return first;
}
...
function evidenceFor(candidate: PlanFixture): ViraTransactionOperationEvidence[] {
  return candidate.operations.map((entry, index: number) => {
...
function freeze(
  candidate: PlanFixture,
  planRevision = 1,
  operationEvidence = evidenceFor(candidate),
) {
...
describe("PROD-10 immutable TransactionPlan", () => {
  ...
  it("canonicalizes object-key order but changes digest when immutable meaning changes", async () => {
    ...
    firstOperation(reordered).actionIntent = {
    ...
    changed.policy.obligations = Object.assign({}, changed.policy.obligations, { ticket: "SEC-42" });
  ...
  it("rejects a plan that outlives the provider trust window used to freeze it", async () => {
    const candidate = plan();
    firstOperation(candidate).trustValidUntilEpochMs = NOW + 120_000;
  ...
  it("rejects floating Action and commercial references", async () => {
    ...
    firstOperation(floatingAction).actionRef.versionRef = "latest";
    ...
    firstPricingRef(floatingPrice).versionRef = "latest";
  ...
  it("rejects cross-tenant delegation and operation SecretRef drift", async () => {
    ...
    firstOperation(wrongSecret).secretRef.projectId = "project-other";
