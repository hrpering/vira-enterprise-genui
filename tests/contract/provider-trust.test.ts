...
describe("PROD-09 provider trust", () => {
  it("trusts only exact healthy evidence for an active canonical connection", () => {
    ...
    expect(result).toMatchObject({
      ...
      value: {
        trusted: true,
        evidenceId: "trust.demo.connection.e001",
        connectionId: "demo.connection",
        providerId: "demo",
  ...
  it("rejects provider, connection, scope and credential drift", () => {
    ...
    const otherScope = { version: "1", organizationId: "org-demo", projectId: "project-other", environment: "staging" } as const;
  ...
  it("reports exact identity failures and rejects malformed or secret-bearing evidence", () => {
    expect(parseViraProviderTrustEvidence({ ...evidence(), id: "trust.demo.connection.001" })).toMatchObject({ ok: false, issue: { code: "INVALID_EVIDENCE", path: "$.id" } });
