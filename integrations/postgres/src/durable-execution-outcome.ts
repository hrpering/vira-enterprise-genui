...
import {
  recordViraDurableExecutionPrivateOutcome,
  type ViraDurableExecutionPrivateOutcome,
} from "../../../packages/durable-execution/src/outcome.js";
...
import type {
  ViraPostgresDurableExecutionMutationResult,
...
export interface ViraPostgresDurableExecutionOutcomeStore {
...
function rowInteger(value: unknown, allowZero = false): number {
  ...
  throw new TypeError("PostgreSQL durable outcome integer is invalid");
}
...
function parseRecord(input: unknown): ViraDurableExecutionRecord {
  ...
  if (!parsed.ok || !isObject(parsed.value) || !exactFields(parsed.value, RECORD_FIELDS)) {
    throw new TypeError("PostgreSQL durable outcome record must be canonical exact-shape JSON");
  }
  ...
  ) throw new TypeError("PostgreSQL durable outcome record identity is invalid");
...
function validateRow(
  ...
  ) throw new TypeError("PostgreSQL durable outcome row conflicts with canonical record");
  return record;
}
...
async function insertOutbox(
  ...
  outcome: ViraDurableExecutionPrivateOutcome,
): Promise<void> {
  ...
  if (!event.ok) throw new TypeError("PostgreSQL durable outcome could not create outbox event");
  ...
  if (inserted.rows.length !== 1) {
    throw new TypeError("PostgreSQL durable outcome outbox identity unexpectedly collided");
  }
}

export function createPostgresDurableExecutionOutcomeStore(
  pool: PostgresPoolLike,
): ViraPostgresDurableExecutionOutcomeStore {
  if (pool === null || typeof pool !== "object" || typeof pool.connect !== "function") {
    throw new TypeError("PostgreSQL durable execution outcome store requires a pool");
  }
  ...
  return Object.freeze({
    async record(input: Parameters<ViraPostgresDurableExecutionOutcomeStore["record"]>[0]) {
      ...
      ) throw new TypeError("PostgreSQL durable execution outcome input is invalid");

      return withTenantTransaction(pool, scope, async (client, transactionScope): Promise<ViraPostgresDurableExecutionMutationResult> => {
        const selected = await client.query<OutcomeRow>(
          `SELECT revision, status, lease_epoch, lease_worker_id, dispatch_state, record,
        ...
        if (selected.rows.length !== 1) throw new TypeError("PostgreSQL durable outcome read returned duplicate state");
        ...
        const next = recordViraDurableExecutionPrivateOutcome({
          record: current,
        ...
        const updated = await client.query<OutcomeRow>(
          `UPDATE vira.durable_execution_state
            ...
            RETURNING revision, status, lease_epoch, lease_worker_id, dispatch_state, record`,
          [
        ...
        if (updated.rows.length !== 1) throw new TypeError("PostgreSQL durable outcome update returned duplicate state");
        const persisted = validateRow(updated.rows[0]!, transactionScope, input.executionId);
        await insertOutbox(client, persisted, input.outcome);
