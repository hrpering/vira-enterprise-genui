...
- `object-store/` — PROD-08 private artifact-byte adapter boundary. It consumes canonical artifact metadata, verifies byte integrity, derives tenant/environment-scoped private object keys, and exposes no signed/public URL authority.

Database shape, OIDC provider details, browser cookie mechanics, and object-store vendor mechanics are infrastructure details. Application, runtime, transaction, ledger, governance, enterprise scope/principal/delegation, artifact identity/lineage, commercial, and provider semantics remain in their canonical package owners.
