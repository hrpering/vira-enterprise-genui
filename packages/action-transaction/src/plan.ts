...
function canonicalJson(value: JsonValue): string {
  ...
  if (!record(value)) throw new TypeError("canonical JSON value must be an object, array or primitive");
...
function parsePolicy(input: JsonValue | undefined): ViraTransactionPlanResult<ViraTransactionPolicySnapshot> {
  ...
  if (input.obligations === undefined) return fail("INVALID_POLICY", "$.policy.obligations", "policy obligations are required");
  const refs: string[] = [];
  const seen = new Set<string>();
...
function parseCommercial(input: JsonValue | undefined): ViraTransactionPlanResult<ViraTransactionCommercialSnapshot> {
  ...
  if (input.preflight === undefined) return fail("INVALID_COMMERCIAL_SNAPSHOT", "$.commercial.preflight", "commercial preflight snapshot is required");
...
function parsePlan(input: unknown): ViraTransactionPlanResult<ViraTransactionPlan> {
  ...
  for (const [index, operationInput] of root.operations.entries()) {
    const operation = parseOperation(operationInput, index, scope.value);
  ...
  if (root.approvalRequirements === undefined) return fail("INVALID_INPUT", "$.approvalRequirements", "approval requirements are required");
...
function verifyOperationEvidence(
  ...
  for (const [index, evidence] of evidenceInput.entries()) {
