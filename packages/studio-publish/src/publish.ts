...
export function prepareStudioPublication(input: {
  ...
  const bindings = validateStudioDocumentBindings(
    ...
    input.bindingSourceCatalog,
  );
  if (!bindings.ok) {
    return publishFailure(
      "INVALID_BINDINGS",
      nestedPath("$.document", bindings.issue.path),
      bindings.issue.message,
    );
  }
  ...
  if (!payloads.ok) {
    return publishFailure(
      "INVALID_FLOW",
      nestedPath("$.document", payloads.issue.path),
      payloads.issue.message,
    );
  }

  const design = validateStudioDesignDocument(payloads.value, input.componentCatalog);
  ...
  if (!flow.ok) {
    return publishFailure(
      "INVALID_FLOW",
      nestedPath("$.document", flow.issue.path),
      flow.issue.message,
    );
  }
  ...
  if (!compiled.ok) {
    return publishFailure(
      "COMPILATION_FAILED",
      nestedPath("$.document", compiled.issue.path),
