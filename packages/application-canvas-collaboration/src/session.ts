...
import {
  ...
  type CreateViraCanvasCollaborationSessionResult,
  type ViraCanvasCollaborationIssue,
  type ViraCanvasCollaborationIssueCode,
  type ViraCanvasCollaborationSession,
  type ViraCanvasCollaborator,
  type ViraCanvasPresence,
  type ViraCanvasPresenceResult,
  type ViraCanvasProposalResult,
  type ViraCanvasReviewResult,
  type ViraCanvasSemanticProposal,
  type ViraCanvasSemanticReview,
  type ViraCanvasApplyProposalResult,
} from "./types.js";
...
export function createViraCanvasCollaborationSession(
  input: unknown,
  ...
  const draft = parseViraCanvasDraft(root.draft);
  ...
  function resolveGraphRef(value: JsonValue | undefined, path: string): Parsed<ViraCanvasGraphRef | null> {
    ...
    if (!graph) return failure("INVALID_PRESENCE", path, "activeGraphRef does not resolve in current Canvas semantics");
  ...
  const session: ViraCanvasCollaborationSession = {
    currentDraft,
    ...
    updatePresence: (inputValue): ViraCanvasPresenceResult => {
      ...
      const next: ViraCanvasPresence = Object.freeze({
        version: VIRA_CANVAS_COLLABORATION_VERSION,
        actorId: actor.value,
    ...
    createProposal: (inputValue): ViraCanvasProposalResult => {
      ...
      const candidateDraft = parseViraCanvasDraft({
      ...
      const baseSerialized = serializeViraCanvasSemantics(current);
      const candidateSerialized = serializeViraCanvasSemantics(candidateDraft.value);
      ...
      if (baseSerialized.value === candidateSerialized.value) {
        return failure("NO_SEMANTIC_CHANGE", "$.semantics", "semantic proposal must change canonical Application semantics");
      }

      const projectionCompatibility = parseViraCanvasDraft({
        schemaVersion: current.schemaVersion,
        draftId: current.draftId,
        editorRevision: current.editorRevision,
        semantics: candidate,
      ...
      const next: ViraCanvasSemanticProposal = Object.freeze({
        version: VIRA_CANVAS_COLLABORATION_VERSION,
    ...
    reviewProposal: (inputValue): ViraCanvasReviewResult => {
      ...
      const next: ViraCanvasSemanticReview = Object.freeze({
        version: VIRA_CANVAS_COLLABORATION_VERSION,
    ...
    applyProposal: (inputValue): ViraCanvasApplyProposalResult => {
      ...
      if (
        ...
        return failure("STALE_REVISION", "$.expectedRevision", "proposal base revision no longer matches current Canvas editorRevision");
      }
      ...
      if (!applied.ok) {
        return failure("APPLY_FAILED", applied.issue.path, applied.issue.message);
      }
