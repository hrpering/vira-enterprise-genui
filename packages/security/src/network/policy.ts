...
function parseRule(value: unknown, index: number):
  ...
  if (!origin) return { ok: false, code: "INVALID_ORIGIN", path: `${base}.origin`, message: "origin must be an exact credential-free HTTPS origin without path, query, or fragment" };
...
function parseRequest(input: unknown):
  ...
  if (!parsedUrl) return { ok: false, code: "INVALID_URL", path: "$.request.url", message: "request URL must be credential-free HTTPS without a fragment" };
