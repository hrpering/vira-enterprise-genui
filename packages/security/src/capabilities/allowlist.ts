...
function policyFailure(
  code: CapabilityAllowlistPolicyValidationCode,
  path: string,
  message: string,
): CapabilityAllowlistPolicyResult {
...
function validCapabilityKey(value: unknown): value is string {
  return typeof value === "string"
    && value.length > 0
    && value.length <= CAPABILITY_ALLOWLIST_KEY_MAX_LENGTH;
}
...
export function evaluateCapabilityAllowlist(
  policyInput: unknown,
  capabilityKeyInput: unknown,
): CapabilityAllowlistEvaluationResult {
  const policy = createCapabilityAllowlistPolicy(policyInput);
  if (!policy.ok) {
    return {
      ok: false,
      issue: {
        ...
        message: "capability allowlist policy is invalid",
      },
    };
  }

  if (!validCapabilityKey(capabilityKeyInput)) {
    return {
      ok: false,
      issue: {
        code: "INVALID_CAPABILITY_KEY",
        path: "$.capabilityKey",
        message: "capability key must be a bounded non-empty string supplied by the validated capability owner",
      },
    };
  }
  ...
  return {
    ok: true,
    value: Object.freeze({
      capabilityKey: capabilityKeyInput,
      decision: policy.value.allowed.includes(capabilityKeyInput) ? "allow" : "deny",
    }),
  };
}
