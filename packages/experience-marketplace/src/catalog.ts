...
function containsExactEntry(
  ...
  for (let index = 0; index < length; index += 1) {
    const descriptor = ownData(entries, STRING_CONVERT(index));
...
function insertCanonicalEntry(
  ...
  while (insertAt < entries.length) {
    const current = ownData(entries, STRING_CONVERT(insertAt));
  ...
  for (let index = entries.length; index > insertAt; index -= 1) {
    const previous = ownData(entries, STRING_CONVERT(index - 1));
    if (!previous) return false;
    defineOwnData(entries, STRING_CONVERT(index), previous.value);
  }
  defineOwnData(entries, STRING_CONVERT(insertAt), entry);
  return true;
}
...
function entryFromManifest(manifest: RegistryManifest): ExperienceMarketplaceEntry | undefined {
  ...
  const entry = OBJECT_CREATE(null) as Record<string, unknown>;
  ...
  return OBJECT_FREEZE(entry) as unknown as ExperienceMarketplaceEntry;
}
...
export function createExperienceMarketplaceCatalog(
  ...
  for (let index = 0; index < parsedListings.value.length; index += 1) {
    const descriptor = ownData(parsedListings.value, STRING_CONVERT(index));
    if (!descriptor || !parsedObject(descriptor.value)) {
  ...
  const catalog: ExperienceMarketplaceCatalog = OBJECT_FREEZE({
