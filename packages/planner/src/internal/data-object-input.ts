export interface PlannerDataObjectIssue {
  readonly path: string;
  readonly reason: string;
}

export type PlannerDataObjectResult =
  | { readonly ok: true; readonly value: Readonly<Record<string, unknown>> }
  | { readonly ok: false; readonly issue: PlannerDataObjectIssue };

function failure(path: string, reason: string): PlannerDataObjectResult {
  return { ok: false, issue: { path, reason } };
}

export function readPlannerDataObject(value: unknown, valuePath = "$" ): PlannerDataObjectResult {
  try {
