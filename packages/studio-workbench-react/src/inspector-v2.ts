...
import type {
  StudioActionPayloadTargetOption,
...
import type { StudioInteractionPayloadSource } from "@vira-enterprise-genui/studio-schema";
import type {
...
const panelStyle = Object.freeze({ padding: 14, display: "grid", gap: 14, minWidth: 280 });
...
function updateItems(
  ...
  prop: string,
  value: unknown,
): { readonly items: readonly unknown[]; readonly changed: boolean } {
  let changed = false;
...
function useSelectedIdentity(session: StudioWorkbenchSession): SelectedIdentity | undefined {
...
function tabButton(
  ...
  tab: InspectorTab,
  active: InspectorTab,
  setActive: (value: InspectorTab) => void,
): ReactElement {
...
function propControl(input: {
  ...
  const help = disabled
    ? createElement("span", { style: { fontSize: 10, color: "#6b7280" } }, "Bound in Data")
    : null;

  if (designControl?.control === "color") {
    const color = typeof value === "string" && /^#[0-9A-Fa-f]{6}$/.test(value)
      ? value
      : "#111827";
  ...
  if (definition.type === "boolean") {
    return createElement("label", {
      key: definition.key,
      style: { ...labelStyle, gridTemplateColumns: "1fr auto", alignItems: "center" },
    },
    createElement("span", null, label),
    createElement("input", {
      type: "checkbox",
      checked: value === true,
      disabled,
      onChange: (event: ChangeEvent<HTMLInputElement>) => update(event.currentTarget.checked),
    }),
    help);
  }
    ...
    return createElement("label", { key: definition.key, style: labelStyle },
      ...
      createElement("select", {
        ...
        onChange: (event: ChangeEvent<HTMLSelectElement>) => {
          update(event.currentTarget.value === "" ? undefined : event.currentTarget.value);
        },
        style: controlStyle,
      },
      definition.required ? null : option("", "Default / unset"),
      ...(definition.options ?? []).map((candidate) => option(candidate, candidate))),
      help);
  }
...
function StaticPropsInspector(props: {
  readonly session: StudioWorkbenchSession;
  readonly design: boolean;
}): ReactElement {
  const selected = useSelectedIdentity(props.session);
  ...
  const component = props.session.componentCatalog().components
    .find((candidate) => candidate.ref === selected.component);
  if (!component) {
    return createElement("div", { style: panelStyle }, "Selected component is not registered in the active brand.");
  }
  ...
  const bound = new Set(
    bindingTargets.ok
      ? bindingTargets.value
          .filter((target) => target.currentSource !== undefined)
          .map((target) => target.prop)
      : [],
  );
  const definitions = component.props
    .filter((definition) => isStudioDesignPropKey(definition.key) === props.design);
  if (definitions.length === 0) {
    return createElement(
      "div",
      { style: panelStyle },
      props.design
        ? "This component has no approved design controls."
        : "This component has no editable content properties.",
    );
  }
  ...
  return createElement("div", { style: panelStyle },
    createElement("div", null,
      ...
      createElement("div", { style: { fontSize: 11, opacity: .6, marginTop: 3 } }, selected.nodeId)),
    ...definitions.map((definition) => propControl({
      definition,
      ...
      update: (value) => dispatch({
        type: "setData",
        data: (previous) => updatePuckProp(previous, selected.puckId, definition.key, value),
      }),
    })));
}
...
function DataInspector(props: {
  ...
  const repeat = props.session.repeatTargets(selected.nodeId);
  const targets = props.session.bindingTargets(selected.nodeId);
  if (!repeat.ok) return createElement("div", { style: panelStyle }, repeat.issue.message);
  if (!targets.ok) return createElement("div", { style: panelStyle }, targets.issue.message);

  const repeatValue = repeat.value.currentSource ? sourceValue(repeat.value.currentSource) : "";
  const repeatRow = createElement("div", { style: groupStyle },
    createElement("strong", null, "Collection / Repeat"),
    createElement("p", { style: { margin: 0, fontSize: 11, color: "#6b7280" } },
      "Repeat this authored node at runtime without expanding the Studio document."),
    createElement("select", {
      value: repeatValue,
      "data-testid": "vira-studio-repeat-source",
      onChange: (event: ChangeEvent<HTMLSelectElement>) => {
        const value = event.currentTarget.value;
        if (value === "") {
          if (repeat.value.currentSource) {
            props.mutate(props.session.clearRepeat({ nodeId: selected.nodeId }));
          }
          return;
        }
        const source = repeat.value.compatibleSources.find((candidate) => sourceValue(candidate) === value);
        if (!source) return;
        props.mutate(props.session.setRepeat({
    ...
    option("", "Not repeated"),
    ...repeat.value.compatibleSources.map((source) => option(sourceValue(source), source.label))));
  ...
  const rows = targets.value.map((target) => {
    ...
    const choices: ReactNode[] = [
      option(
        "",
        target.currentSource && target.required ? "Binding required" : "Static / unbound",
        Boolean(target.currentSource && target.required),
      ),
    ...
    return createElement("label", { key: target.prop, style: labelStyle },
      ...
      createElement("select", {
        value: currentValue,
        "data-testid": `vira-studio-binding-${target.prop}`,
        onChange: (event: ChangeEvent<HTMLSelectElement>) => {
          const value = event.currentTarget.value;
          if (value === "") {
          ...
          if (!source) return;
          props.mutate(props.session.setBinding({
            nodeId: selected.nodeId,
            prop: target.prop,
            source: { kind: source.kind, path: source.path },
          }));
  ...
  return createElement("div", { style: panelStyle },
    repeatRow,
    createElement("strong", null, "Data bindings"),
    targets.value.length === 0
      ? createElement("span", { style: { fontSize: 11, color: "#6b7280" } }, "This component has no bindable properties.")
      : null,
    ...rows);
}

function routeValue(
  event: StudioFlowEventOption,
  outcome: (typeof outcomes)[number],
): string {
  return event.routes.find((route) => route.outcome === outcome)?.viewId ?? "";
}

function defaultLiteral(type: StudioBindingValueType): string | number | boolean {
  if (type === "number") return 0;
  if (type === "boolean") return false;
  return "";
}

function payloadSourceValue(source: StudioInteractionPayloadSource | undefined): string {
  if (!source) return "";
  if (source.kind === "literal") return "$literal";
  return `${source.kind}:${source.path}`;
}

function literalEditor(input: {
  readonly target: StudioActionPayloadTargetOption;
  readonly disabled: boolean;
  readonly update: (source: StudioInteractionPayloadSource) => void;
}): ReactElement | null {
  if (input.target.currentSource?.kind !== "literal") return null;
  const value = input.target.currentSource.value;
  if (input.target.valueType === "boolean") {
    return createElement("label", { style: { ...labelStyle, gridTemplateColumns: "1fr auto" } },
      createElement("span", null, "Literal value"),
      createElement("input", {
        type: "checkbox",
        checked: value === true,
        disabled: input.disabled,
        onChange: (event: ChangeEvent<HTMLInputElement>) => {
          input.update({ kind: "literal", value: event.currentTarget.checked });
        },
      }));
  }
  return createElement("input", {
    type: input.target.valueType === "number" ? "number" : "text",
    value: typeof value === "string" || typeof value === "number" ? value : "",
    disabled: input.disabled,
    "aria-label": `${input.target.key} literal value`,
    onChange: (event: ChangeEvent<HTMLInputElement>) => {
      if (input.target.valueType === "number") {
        const parsed = Number(event.currentTarget.value);
        if (Number.isFinite(parsed)) input.update({ kind: "literal", value: parsed });
      } else {
        input.update({ kind: "literal", value: event.currentTarget.value });
      }
    },
    style: controlStyle,
  });
...
function PayloadRows(props: {
  readonly session: StudioWorkbenchSession;
  readonly selected: SelectedIdentity;
  readonly event: StudioFlowEventOption;
  readonly mutate: MutationReporter;
}): ReactElement | null {
  const targets = props.session.actionPayloadTargets(props.selected.nodeId, props.event.event);
  if (!targets.ok) {
    return createElement("div", { role: "alert", style: { fontSize: 11 } }, targets.issue.message);
  }
  if (targets.value.length === 0) return null;

  return createElement("div", { style: { display: "grid", gap: 10 } },
    createElement("strong", { style: { fontSize: 11 } }, "Action payload"),
    ...targets.value.map((target) => {
      const currentValue = payloadSourceValue(target.currentSource);
      const disabled = !props.event.currentActionEvent;
      const set = (source: StudioInteractionPayloadSource) => {
        props.mutate(props.session.setActionPayload({
          nodeId: props.selected.nodeId,
          event: props.event.event,
          key: target.key,
          source,
        }));
      };
      return createElement("label", { key: target.key, style: labelStyle },
        createElement("span", null, `${target.key}${target.required ? " • required" : ""}`),
        createElement("select", {
          value: currentValue,
          disabled,
          "data-testid": `vira-studio-action-payload-${props.event.event}-${target.key}`,
          onChange: (event: ChangeEvent<HTMLSelectElement>) => {
            const value = event.currentTarget.value;
            if (value === "") {
              if (target.currentSource) {
                props.mutate(props.session.clearActionPayload({
                  nodeId: props.selected.nodeId,
                  event: props.event.event,
                  key: target.key,
                }));
            ...
            if (value === "$literal") {
              set({ kind: "literal", value: defaultLiteral(target.valueType) });
              return;
            }
            const source = target.compatibleSources.find((candidate) => sourceValue(candidate) === value);
            if (!source) return;
            set({ kind: source.kind, path: source.path });
          },
          style: controlStyle,
        },
        option("", target.required ? "Not mapped — publish blocked" : "Not mapped"),
        option("$literal", "Literal value"),
        ...target.compatibleSources.map((source) => option(sourceValue(source), source.label))),
        literalEditor({ target, disabled, update: set }));
    }));
}

function ActionsInspector(props: {
  readonly session: StudioWorkbenchSession;
  readonly mutate: MutationReporter;
}): ReactElement {
  const selected = useSelectedIdentity(props.session);
  ...
  if (options.value.events.length === 0) {
    return createElement("div", { style: panelStyle }, "This component exposes no actions.");
  }
  ...
  const groups = options.value.events.map((event) => {
      ...
      return createElement("label", { key: outcome, style: labelStyle },
        ...
        createElement("select", {
          ...
          onChange: (change: ChangeEvent<HTMLSelectElement>) => {
            ...
            if (targetViewId === "") {
              if (currentRoute) {
                props.mutate(props.session.clearRoute({
                  nodeId: selected.nodeId,
                  event: event.event,
                  outcome,
                }));
            ...
            props.mutate(props.session.setRoute({
              nodeId: selected.nodeId,
              event: event.event,
              outcome,
              targetViewId,
    ...
    return createElement("div", { key: event.event, style: groupStyle },
      ...
      createElement("label", { style: labelStyle },
        ...
        createElement("select", {
          value: event.currentActionEvent ?? "",
          onChange: (change: ChangeEvent<HTMLSelectElement>) => {
            const actionEvent = change.currentTarget.value;
            if (actionEvent === "") {
              if (event.currentActionEvent) {
                props.mutate(props.session.clearAction({ nodeId: selected.nodeId, event: event.event }));
              }
              return;
            }
            props.mutate(props.session.setAction({
              nodeId: selected.nodeId,
              event: event.event,
              actionEvent,
            }));
  ...
  return createElement("div", { style: panelStyle },
    createElement("strong", null, "Actions, payloads & routes"),
    ...groups);
}

export function ViraStudioInspectorV2(props: {
  readonly session: StudioWorkbenchSession;
  readonly mutate: MutationReporter;
}): ReactElement {
  const [tab, setTab] = useState<InspectorTab>("content");
  return createElement("div", {
    "data-testid": "vira-studio-inspector",
    style: { minHeight: 0 },
  },
  createElement("div", {
    style: {
