...
export function createStudioWorkbenchSession(input: CreateStudioWorkbenchSessionInput): CreateStudioWorkbenchSessionResult {
  ...
  let puckSession: StudioPuckAuthoringSession;

  function resetPuck(): StudioWorkbenchIssue | undefined {
    const created = createStudioPuckAuthoringSession({ document: current, catalog: componentCatalog, viewId: activeViewId, allocateNodeId: input.allocateNodeId });
    if (!created.ok) return issue("INVALID_PUCK_SESSION", created.issue.path, created.issue.message);
    puckSession = created.value;
    return undefined;
  }

  const initialPuckIssue = resetPuck();
  if (initialPuckIssue) return { ok: false, issue: initialPuckIssue };

  function commit(candidate: unknown): StudioWorkbenchDocumentResult {
    ...
    current = flow.value;
    const puckIssue = resetPuck();
    return puckIssue ? { ok: false, issue: puckIssue } : success(current);
  }
  ...
  const session: StudioWorkbenchSession = Object.freeze({
    ...
    removeView: (viewId: string) => {
      ...
      if (activeViewId === viewId) activeViewId = current.entryView;
      return commit({
        ...current,
