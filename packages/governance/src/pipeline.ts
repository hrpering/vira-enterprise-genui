...
function parseContext(input: unknown): ViraGovernanceContext | undefined {
  ...
  for (const required of ["version", "instanceId", "experienceId", "experienceVersion", "platform", "actionIntent"]) {
  ...
  if (
    ...
    || !bounded(parsed.value.experienceId)
    || !bounded(parsed.value.experienceVersion, 256)
    || typeof parsed.value.platform !== "string"
    || !PLATFORMS.has(parsed.value.platform)
  ) return undefined;
  ...
  if (
    intent.version !== "1"
    || intent.instanceId !== parsed.value.instanceId
    || typeof intent.expectedStateRevision !== "number"
    || !Number.isSafeInteger(intent.expectedStateRevision)
    || intent.expectedStateRevision < 0
    || !bounded(intent.idempotencyKey, 256)
  ) return undefined;
  const intentKeys = Object.keys(intent);
  if (intentKeys.length !== 5 || !["version", "instanceId", "expectedStateRevision", "idempotencyKey", "action"].every((key) => Object.hasOwn(intent, key))) return undefined;
...
function parseObligations(input: JsonValue | undefined, allowed: ReadonlySet<string>): ViraGovernanceObligation[] | undefined {
  ...
  for (const raw of input) {
    ...
    if (Object.hasOwn(raw, "params")) {
      if (!isObject(raw.params)) return undefined;
      params = raw.params;
    }
...
function parseVerdict(input: unknown, expectedProvider: string, allowedObligations: ReadonlySet<string>): ViraGovernanceVerdict | undefined {
  ...
  if (
    parsed.value.version !== VIRA_GOVERNANCE_VERSION
    || typeof parsed.value.effect !== "string"
    || !EFFECTS.has(parsed.value.effect)
    || !bounded(parsed.value.reasonCode, 256)
    || parsed.value.provider !== expectedProvider
  ) return undefined;
  ...
  if (parsed.value.effect === "transform") {
    if (!isObject(transformedPayload)) return undefined;
  } else if (transformedPayload !== undefined) return undefined;
  return Object.freeze({
    version: VIRA_GOVERNANCE_VERSION,
    effect: parsed.value.effect as ViraGovernanceVerdict["effect"],
    reasonCode: parsed.value.reasonCode,
    obligations: Object.freeze(obligations),
    provider: expectedProvider,
    ...(evidenceRef === undefined ? {} : { evidenceRef: evidenceRef as string }),
    ...(transformedPayload === undefined ? {} : { transformedPayload: transformedPayload as JsonObject }),
  });
}
...
function makeChallenge(context: ViraGovernanceContext, verdict: ViraGovernanceVerdict): ViraApprovalChallenge {
  return Object.freeze({
    version: VIRA_GOVERNANCE_VERSION,
    challengeId: `challenge:${verdict.provider}:${context.actionIntent.action.id}`,
    instanceId: context.instanceId,
    actionId: context.actionIntent.action.id,
    actionType: context.actionIntent.action.type,
...
function parseApproval(input: unknown, challenge: ViraApprovalChallenge): ViraApprovalDecision | undefined {
  ...
  if (
    parsed.value.version !== VIRA_GOVERNANCE_VERSION
    || parsed.value.challengeId !== challenge.challengeId
    || (parsed.value.decision !== "approved" && parsed.value.decision !== "denied")
  ) return undefined;
...
function transformedContext(context: ViraGovernanceContext, payload: JsonObject): ViraGovernanceContext | undefined {
  const action = createRuntimeAction({
    id: context.actionIntent.action.id,
    type: context.actionIntent.action.type,
    source: context.actionIntent.action.source,
    payload,
  });
  if (!action.ok) return undefined;
  return Object.freeze({
    ...context,
    actionIntent: Object.freeze({ ...context.actionIntent, action: action.value }),
  });
}
...
export function createViraGovernancePipeline(input: ViraGovernancePipelineInput): ViraGovernancePipelineCreateResult {
  if (input === null || typeof input !== "object" || !Array.isArray(input.providers) || !Array.isArray(input.allowedObligations)) {
    return { ok: false, issue: issue("INVALID_PIPELINE", "$", "governance pipeline input is invalid") };
  }
  if (input.providers.length > VIRA_GOVERNANCE_MAX_PROVIDERS || input.providers.some((provider) => !validProvider(provider))) {
    return { ok: false, issue: issue("INVALID_PROVIDER", "$.providers", "GovernanceProvider identity is invalid") };
  }
  const providerIds = input.providers.map((provider) => provider.id);
  if (new Set(providerIds).size !== providerIds.length || !validApprovalProvider(input.approvalProvider)) {
    return { ok: false, issue: issue("INVALID_PIPELINE", "$", "governance providers must be unique and approval provider valid") };
  }
  if (input.allowedObligations.length > VIRA_GOVERNANCE_MAX_OBLIGATIONS || input.allowedObligations.some((id) => typeof id !== "string" || !isSemanticNamespace(id))) {
    return { ok: false, issue: issue("INVALID_PIPELINE", "$.allowedObligations", "trusted obligation catalog is invalid") };
  }
  const allowedObligations = new Set(input.allowedObligations);
  if (allowedObligations.size !== input.allowedObligations.length) {
    return { ok: false, issue: issue("INVALID_PIPELINE", "$.allowedObligations", "trusted obligation catalog contains duplicates") };
  }
  ...
  const pipeline: ViraGovernancePipeline = {
    ...
    async evaluate(evaluation: ViraGovernanceEvaluationInput): Promise<ViraGovernanceEvaluationResult> {
      ...
      if (coreSafety.effect === "deny") {
        return { ok: false, issue: issue("CORE_SAFETY_DENIED", "$.coreSafety", "Vira Core Safety deny cannot be overridden") };
      }
      ...
      for (let index = 0; index < providers.length; index += 1) {
        ...
        let raw: unknown;
        try {
          raw = await provider.evaluate(context);
        } catch {
          return { ok: false, issue: issue("PROVIDER_FAILED", `$.providers[${index}]`, "GovernanceProvider failed closed") };
        }
        const verdict = parseVerdict(raw, provider.id, allowedObligations);
        if (!verdict) {
          return { ok: false, issue: issue("INVALID_VERDICT", `$.providers[${index}]`, "GovernanceProvider returned an invalid verdict or obligation") };
        }
        verdicts.push(verdict);

        if (verdict.effect === "deny") {
          return { ok: false, issue: issue("GOVERNANCE_DENIED", `$.providers[${index}]`, "governance provider denied the protected action") };
        }
        ...
        if (verdict.effect === "challenge") {
          ...
          if (!approvalProvider) {
            return { ok: false, issue: issue("APPROVAL_REQUIRED", `$.providers[${index}]`, "governance challenge requires an ApprovalProvider"), challenge };
          }
          let rawApproval: unknown;
          try {
            rawApproval = await approvalProvider.decide(challenge);
          } catch {
            return { ok: false, issue: issue("APPROVAL_FAILED", "$.approvalProvider", "ApprovalProvider failed closed"), challenge };
          }
          const approval = parseApproval(rawApproval, challenge);
          ...
          if (approval.decision === "denied") {
            return { ok: false, issue: issue("GOVERNANCE_DENIED", "$.approvalProvider", "approval challenge was denied"), challenge };
          }
        }
      }
