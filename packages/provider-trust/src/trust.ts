...
type Failure = { readonly ok: false; readonly issue: ViraProviderTrustIssue };
type ParsedScope = { readonly ok: true; readonly value: ViraEnterpriseScope; readonly context: ViraEnterpriseContext } | Failure;
type ParsedConnection = { readonly ok: true; readonly scope: ViraEnterpriseScope; readonly credentialRef: ViraSecretRef } | Failure;

function fail(code: ViraProviderTrustIssueCode, path: string, message: string): Failure {
...
function canonicalScope(scopeValue: JsonValue | undefined): ParsedScope {
...
function canonicalConnectionIdentity(connection: ViraProviderConnection): ParsedConnection {
  if (
    connection === null
    ...
    || typeof connection.scope !== "object"
    || Object.keys(connection.scope).length !== SCOPE_FIELDS.length
    || typeof connection.scope.organizationId !== "string"
    || typeof connection.scope.projectId !== "string"
    || typeof connection.scope.environment !== "string"
...
export function parseViraProviderTrustEvidence(input: unknown): ViraProviderTrustEvidenceResult {
  ...
  if (root.version !== VIRA_PROVIDER_TRUST_VERSION) return fail("INVALID_EVIDENCE", "$.version", "provider trust evidence version is invalid");
  if (typeof root.id !== "string" || !isSemanticNamespace(root.id)) return fail("INVALID_EVIDENCE", "$.id", "provider trust evidence id must be a semantic namespace");
  if (typeof root.connectionId !== "string" || !isSemanticNamespace(root.connectionId)) return fail("INVALID_EVIDENCE", "$.connectionId", "provider trust connectionId must be a semantic namespace");
  if (typeof root.providerId !== "string" || !isSemanticNamespace(root.providerId)) return fail("INVALID_EVIDENCE", "$.providerId", "provider trust providerId must be a semantic namespace");
...
export function evaluateViraProviderTrust(input: ViraProviderTrustEvaluationInput): ViraProviderTrustDecisionResult {
  if (input === null || typeof input !== "object") return fail("INVALID_EVIDENCE", "$", "provider trust evaluation input is invalid");
