...
Wildcard hostnames are rejected after URL parsing/normalization, including wildcard characters introduced by percent-decoding (for example `%2A`). This prevents a syntactically parseable wildcard host from being mistaken for an exact origin.
